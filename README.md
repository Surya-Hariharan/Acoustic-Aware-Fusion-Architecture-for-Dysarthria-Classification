...
|---|---|---|---|
| Learned | Raw waveform (speech-focused VAD profile) | wav2vec 2.0 + LoRA (`q_proj/k_proj/v_proj` in each encoder layer), masked mean-pool, projected 768→128 | 128-dim **Z_learned** |
...
| Suprasegmental | Framewise F0 (semitones on observed voiced frames; zero when unvoiced) + voicing mask + intensity, 3ch/frame, **temporal-preserving** VAD profile (wider padding margin — see `src.preprocessing.load_and_preprocess_supra`) | 2-layer 1D-CNN, masked mean-pool, projected →64 | 64-dim **Z_supra** |
