...
def load_manifest() -> pd.DataFrame:
    ...
    Regenerates it from raw audio if missing, so train.py is runnable
    standalone without requiring the notebook to have been run first.
    """
...
def build_loaders(train_df: pd.DataFrame, val_df: pd.DataFrame, test_df: pd.DataFrame,
    ...
    train_loader = DataLoader(
        UASpeechDataset(train_df), batch_size=batch_size, shuffle=True,
        num_workers=num_workers, pin_memory=pin_memory, drop_last=len(train_df) > batch_size)
    val_loader = DataLoader(
        UASpeechDataset(val_df), batch_size=batch_size, shuffle=False,
        num_workers=num_workers, pin_memory=pin_memory)
    test_loader = DataLoader(
        UASpeechDataset(test_df), batch_size=batch_size, shuffle=False,
        num_workers=num_workers, pin_memory=pin_memory)
