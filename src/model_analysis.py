...
def compute_shap_values(features_df: pd.DataFrame, task: str = "detection",
    ...
    Returns (shap_values, X_sample, feature_columns, class_names, surrogate_model).
    shap_values is the positive-class ("Dysarthric Patient") contribution
    array for "detection"; for "severity" it is a list, one array per class.
    """
    import shap
    ...
    if task == "detection" and isinstance(raw_values, list):
        shap_values = raw_values[1]
    else:
        shap_values = raw_values

    return shap_values, X_sample, feature_columns, class_names, surrogate


def plot_shap_summary(shap_values, feature_columns, title: str = "SHAP Feature Importance",
                      show: bool = False) -> str:
    """
    Bar chart of mean |SHAP value| per feature, read directly off the values
    compute_shap_values() already produced — this is the "feature importance
    analysis" item in the target notebook structure, folded in here rather
    than built as separate machinery, since SHAP's own output already ranks
    exactly this.
    """
    ...
    ax.barh(ranked_features[::-1], ranked_values[::-1], color="#4c72b0")
    ax.set_xlabel("mean |SHAP value|")
    ax.set_title(title, fontsize=14)
    fig.tight_layout()
    return _finish(fig, "shap_feature_importance.png", show)
